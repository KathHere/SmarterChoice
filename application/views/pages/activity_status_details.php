	...
	<div class="wrapper-md">
	    ...
	    <div class="panel-report panel panel-default">
		    <div class="panel-heading">
		      	<?php
		      		if($activity_status_name == "NewPT")
		      		{
		      			echo "New Customer";
		      		}
		      		else if($activity_status_name == "NewItem")
		      		{
		      			echo "New Item";
		      		}
		      		...
		      		else if($activity_status_name == "PTMove")
		      		{
		      			echo "CUS Move";
		      		}
		      		else
		    ...
		    <div class="panel-body">
				...
				<div class="col-xs-12 col-sm-12 col-md-12 hidden-print" style="margin-top:20px;">
					<div class="col-xxs-12 col-xs-6 col-sm-6 col-md-6">
						<span class="pull-right">
							From
							<input type="text" class="form-control choose_date filter_from" id="" placeholder="" style="width: 110px;margin-top: -26px;margin-left: 40px;" value="<?php echo date("m-d-Y"); ?>" disabled="true">
						</span>
					</div>
					<div class="col-xxs-12 col-xs-6 col-sm-6 col-md-6">
						<span class="pull-left">
							To
							<input type="text" class="form-control choose_date filter_to" id="" placeholder="" style="width: 110px;margin-top: -26px;margin-left: 25px;" value="<?php echo date("m-d-Y"); ?>" disabled="true">
						</span>
					</div>
				</div>
				...
				<div class="col-xs-12 col-sm-12 col-md-12" style="margin-top:8px;">
					...
					<div class="col-xxs-12 col-xs-6 col-sm-6 col-md-6" style="text-align:center;">
			            ...
			            ?>
								<select class="form-control pull-center filter_activity_status_details select2-ready" name="filter_activity_status_details" style="margin-top: 6px;border: 0px;font-size: 16px;font-weight: 600;text-align-last:center;" disabled="true">
				...
				?>
					<div class="pagination-container">
						<nav aria-label="...">
							<ul class="pager">
								<!-- <li class="previous disabled hidden-print"><a href="javascript:;"><span aria-hidden="true">&larr;</span> Previous</a></li>
								<li class="next hidden-print"><a href="javascript:;">Next <span aria-hidden="true">&rarr;</span></a></li> -->
							</ul>
						</nav>
					</div>
				</div>
...
<script type="text/javascript">
	...
	$(document).ready(function(){
		var objtemp = [ 
			...
			{ first_nom: 'Alab', last_nom: 'Subraman' },
			{ first_nom: 'Robert', last_nom: 'Downey' },
			{ first_nom: 'Felix', last_nom: 'Celi' },
			{ first_nom: 'Dan', last_nom: 'Celi' }
		];
		var tempobj = objtemp.sort(function(a,b){
		...
		var sortReportList = function(objlist) {
			return objlist.sort(function(a,b){
				if(a.p_lname< b.p_lname) return -1;
				if(a.p_lname >b.p_lname) return 1;
				if(a.p_fname< b.p_fname) return -1;
				if(a.p_fname >b.p_fname) return 1;
				return 0;
			});
		};
		...
		var activity_status_content = function(page){
			...
			{
				var temp_filter_from = filter_from.split("-");
				filter_from = temp_filter_from[2] + '-' + temp_filter_from[0] + '-' + temp_filter_from[1];

				var temp_filter_to = filter_to.split("-");
				filter_to = temp_filter_to[2] + '-' + temp_filter_to[0] + '-' + temp_filter_to[1];
					...
					if(obj.sign_here == 1)
					{
						for(var val in obj.patient_list)
						{
							if(obj.patient_list[val].organization_id != undefined) {
								temp_html += '<tr>'+
									'<td> <a class="hidden-print" href="'+base_url+'order/patient_profile/'+obj.patient_list[val].medical_record_id+'/'+obj.patient_list[val].organization_id+'" target="_blank"> '+obj.patient_list[val].p_lname+', '+obj.patient_list[val].p_fname+' </a> <span class="show-print" style="display: none">'+obj.patient_list[val].p_lname+', '+obj.patient_list[val].p_fname+'</span></td>'+
									'<td> <a class="hidden-print" href="'+base_url+'order/patient_profile/'+obj.patient_list[val].medical_record_id+'/'+obj.patient_list[val].organization_id+'" target="_blank"> '+obj.patient_list[val].medical_record_id+' </a><span class="show-print" style="display: none">'+obj.patient_list[val].medical_record_id+'</span></td>'+
									'</tr>';
							} else {
								temp_html += '<tr>'+
									'<td> <a class="hidden-print" href="'+base_url+'order/patient_profile/'+obj.patient_list[val].medical_record_id+'/'+obj.patient_list[val].ordered_by+'" target="_blank"> '+obj.patient_list[val].p_lname+', '+obj.patient_list[val].p_fname+' </a><span class="show-print" style="display: none">'+obj.patient_list[val].p_lname+', '+obj.patient_list[val].p_fname+'</span></td>'+
									'<td> <a class="hidden-print" href="'+base_url+'order/patient_profile/'+obj.patient_list[val].medical_record_id+'/'+obj.patient_list[val].ordered_by+'" target="_blank"> '+obj.patient_list[val].medical_record_id+' </a><span class="show-print" style="display: none">'+obj.patient_list[val].medical_record_id+'</span></td>'+
									'</tr>';
							}
		...
		var temp_list_global = [];
		var temp_page_global = 1;

		var activity_status_content_lazy_load = function(page){
			var filter_from = $("body").find(".filter_from").val();
			var filter_to = $("body").find(".filter_to").val();
			var hospiceID = $("body").find(".filter_activity_status_details").val();
			var status_name = $("body").find("#activity_status_name").val();
			...
			if(filter_from != 0 && filter_to != 0)
			{
				...
				if(viewed_activty_status_type == 1)
				{
					activity_status_tbody.append("<tr id='load_spinner_tr'><td colspan='2' style='text-align:center;'>Retrieving Data... <i class='fa fa-spin fa-spinner'></i></td></tr>");
				}
				else
				{
					activity_status_tbody.append("<tr id='load_spinner_tr'><td colspan='3' style='text-align:center;'>Retrieving Data... <i class='fa fa-spin fa-spinner'></i></td></tr>");
				}
				...
				if (pagenum > temp_page_global) {
					console.log('done fetching!!!!!!', temp_list_global);
					$("#load_spinner_tr").remove();
					...
					// activity_status_tbody.html(temp_html_global);
				} else {
					...
					$.post(base_url+"report/sort_activity_status_details/" + filter_from +"/"+ filter_to +"/"+ hospiceID +"/"+ status_name+"/"+pagenum,"", function(response){
						...
						obj.pagination_details.total_records = obj.pagination_details.total_records*1;
						obj.pagination_details.current_page = obj.pagination_details.current_page*1;
						temp_page_global = obj.pagination_details.total_pages;
						console.log('obj.pagination_details.total_pages', obj.pagination_details.total_pages);
						console.log('obj.page', pagenum);
						if(obj.pagination_details.total_pages < 2){
							$('.pagination-container').addClass("hidden");
						}
						else{
						...
						temp_list_global = [...temp_list_global, ...obj.patient_list];
						if (temp_list_global.length > 0) {
							temp_list_global = sortReportList(temp_list_global);
							obj.patient_list = temp_list_global;
						}
						...
						activity_status_content_lazy_load(pagenum+1);
					});
				}
		...
		activity_status_content_lazy_load();
		...
		$('body').on('click','.print_activity_status',function(){
          	...
          	window.open(base_url+'report/print_activity_status_details_v2/'+filter_from+'/'+filter_to+'/'+hospiceID+'/'+status_name);
	  	});
