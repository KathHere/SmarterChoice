...
.customer_order_summary_table
{
  border-top: 0px !important;
  margin-top: -10px !important;
}

.customer_order_summary_panel_body
{
...
<div class="wrapper-md">
  <div class="patient-profile">
    <div class="panel panel-default">
      <div class="panel-body" style="padding-top:0;padding-bottom:0;">
        <div class="row">
          <?php if($this->session->userdata('account_type') == "dme_admin" || $this->session->userdata('account_type') == 'dme_user' || $this->session->userdata('account_type') == "biller" || $this->session->userdata('account_type') == "customer_service" || $this->session->userdata('account_type') == "rt" || $this->session->userdata('account_type') == "distribution_supervisor") :?>
            <div class="col-xs-12" style="background-color:#fafafa; padding:20px;">
              <div class="media">
                ...
                <div class="media-right mt10">
                  ...
                  <a href="javascript:void(0)" class="edit_patient_profile" data-organization-id="<?php echo $information['ordered_by'] ?>" data-id="<?php echo $information['medical_record_id'] ?>"D><i class="icon-pencil"></i> Edit Info</a>&nbsp &nbsp &nbsp <br />
                  <?php } ?>
                  <a href="javascript:void(0)" id="add_patient_notes" class="patient_notes_count" data-patient-id="<?php echo $information['patientID'] ?>" data-id="<?php echo $information['medical_record_id'] ?>" data-fname="<?php echo $information['p_fname'] ?>" data-lname="<?php echo $information['p_lname'] ?>" data-hospice="<?php echo $information['hospice_name'] ?>" title=""><i class="icon icon-speech"></i> <input type="hidden" class="customer_notes_count_input" value="<?php echo $note_counts?>">
                    <span class="customer_notes_count"> <?php echo $note_counts ?> </span> Customer Notes</a>
                </div>
              </div>
            </div>
          <?php endif;?>
          <div class="col-xs-12">
            <h4>Customer Medical Record # <?php echo $information['medical_record_id'] ?> <?php if(!empty($recurring_order)){?> <img src="<?php echo base_url(); ?>assets/img/recurring.png"  style="width: 20px;height: 19px;" /> <?php } ?></h4>
            ...
            ?>
                ...
                <div style="width:110px !important;"> <span>
                  ...
                  <div style="width:400px !important;margin-top: -14px;margin-left: <?php echo $margin_left_css; ?>;">
                    <input type="hidden" value="<?php echo $information['is_active']?>" id="patient_is_active">
                    <span style="margin-left:44px;">
                      CUS Days:
                      <?php
                        if ($this->session->userdata('account_type') != "hospice_user" && $this->session->userdata('account_type') != "hospice_admin") {
                      ?>
                        <a href="javascript:;"
                            id="patient_days"
                            data-pk="<?php echo $information['patientID'] ?>"
                            data-url="<?php echo base_url('order/update_patient_days/'.$information['patientID'].'/'.$information['patient_days']); ?>"
                            data-title="Enter Patient Days"
                            data-value="<?php echo $information['patient_days']; ?>"
                            data-type="number"
                            class="data_tooltip editable editable-text"
                        >
                          <span class="under-line"> <?php echo $information['patient_days']; ?> </span>
                          <!-- <span class=""> <?php echo $information['patient_days']; ?> </span> -->
                        </a>
                        <?php 
                          // echo $cus_day_order_type.'-'.$cus_day_date_type.'-'.$cus_day_dispay_date;
                        ?>
                        &nbsp;
                        <?php
                            if ($cus_day_dispay_date != '' && $cus_day_dispay_date != '0000-00-00') {
                        ?>
                        <a href="javascript:;"
                            id="patient_days_date"
                            data-pk="<?php echo $information['patientID'] ?>"
                            data-url="<?php echo base_url('order/update_patient_days_v2/'.$information['patientID'].'/'.$information['patient_days'].'/'.$cus_day_order_type.'/'.$cus_day_date_type.'/'.$cus_day_uniqueID.'/'.$cus_day_pickedup_uniqueID); ?>"
                            data-title="Enter Date"
                            data-value="<?php echo date("Y-m-d", strtotime($cus_day_dispay_date)); ?>"
                            data-type="combodate"
                        ...
                        >
                          <?php echo date("m/d/Y", strtotime($cus_day_dispay_date)); ?>
                        </a>
                        &nbsp;&nbsp;
                        <button type="button" id="hide_btn_cus_date" class="btn btn-default btn-sm" style="display: none !important">
                          <i class="glyphicon glyphicon-remove"></i>
                        </button>
                        <span class="visible-print-block"> <?php echo $information['patient_days']; ?> </span>
                      <?php
                          }
                        } else {
                      ?>
  ...
  <?php
          if(empty($information['p_weight']) && empty($information['p_height'])) :
  ?>
...
<?php if ($this->session->userdata('account_type') != 'dispatch') { ?>
  ...
  <div class="col-md-12">
    <div class="row">
      <div class="panel panel-default">
        ...
        <div class="panel-body customer_order_summary_panel_body">

          <div class="table-responsive">
            <table class="table customer_order_summary_table" id="equipment_summary_tbl">
              <thead>
                <tr>
...
<?php
  $returned_result = '';
  $queried_data = get_patients_first_order_uniqueID_v2($unique_id,$hospiceID);
  $returned_result = $queried_data['order_uniqueID'];
?>
<input type="hidden" id="first_workorder_uniqueID" value="<?php echo $returned_result; ?>">
...
<script type="text/javascript">
  $(document).ready(function(){
    ...
    var firstWorkorderUniqueID = $('#first_workorder_uniqueID').val();

    var datatable = $('.customer_order_summary_table').DataTable({
        "language": {
           "processing": "Retrieving item list. Please wait..."
        },
        "bLengthChange": false,
        "pageLength": 10,
        "processing": true,
        ...
        "searching": false,
        "ajax": {
            url: base_url+"order/get_customer_order_summary_data/"+ medical_record_id +"/"+ hospiceID +"/"+ patientID + "/" + firstWorkorderUniqueID
        },
        "columns": [
            { "data": "delivery_date_data" },
            { "data": "delivery_wo_number_data" },
            { "data": "activity_type_data" },
            { "data": "item_number_data" },
            { "data": "item_description_data" },
            { "data": "qty_data" },
            { "data": "serial_lot_number_data" },
            { "data": "pickup_wo_number_data" },
            { "data": "pickup_date_data" },
            { "data": "item_type_data" },
            { "data": "item_location_data" }
        ],
        "order": [[ 0, 'desc' ]],
        "columnDefs":[
            {
            ...
            },
            {
                "targets": 4,
                "orderable": false,
                "className": "item_description_column"
            },
            {
                "targets": 5,
                "orderable": false
        ...
        fnDrawCallback: function( oSettings ) {
          $('body').find('.cancel_item_btn').on('click',function(){
            console.log('nakasulod kos func');
            var uniqueID = $(this).attr("data-unique-id");
            var equipmentID = $(this).attr("data-equipment-id");
            var patientID = $(this).attr("data-patient-id");
            var originalActivityTypeID = $(this).attr("data-original-activity-id");
            var activityTypeID = $(this).attr("data-activity-id");
            var orderID = $(this).attr('data-order-id');
            var medicalRecordID = $(this).attr('data-medical-record-id');
            var patientAddress = $(this).attr('data-patient-address');
            var orderStatus = $(this).attr('data-order-status');

            if(orderStatus != "confirmed") {
              jAlert("Not Applicable.",'Reminder');
            }
            else if(originalActivityTypeID != 2 && originalActivityTypeID != 3 && activityTypeID != 2 && activityTypeID != 3)
            {
              jConfirm("Are you sure you want to cancel this item?","Reminder",function(response){
                if(response)
                {
                  $.get(base_url+'order_extend/cancel_confirmed_item/'+orderID+'/'+equipmentID+'/'+uniqueID+'/'+patientID+'/'+activityTypeID+'/'+originalActivityTypeID+'/'+patientAddress, function(response){
                    var obj = $.parseJSON(response);
                    console.log('workorders', obj);
                    me_message_v2({error:obj['error'],message:obj['message']});
                    var current_customer_notes_count = parseInt($('.customer_notes_count_input').val());
                    current_customer_notes_count += 1;
                    $('.customer_notes_count_input').val(current_customer_notes_count);
                    $('.customer_notes_count').html(current_customer_notes_count);
                    if(obj['error'] == 0) {
                      datatable.ajax.reload();
                      $('.comment_'+orderID).click();
                      if(obj['is_deactivated_account'] == 1) {
                        $('.additional_equip_button').attr('disabled', 'disabled');
                        // $('.activity_type_section_btn').prop('disabled', true);
                        $('.activity_type_section_btn').show();
                      }
                      if(obj['get_work_order_items'] == 0) {
                        /*
                        | If all the items in a workorder is cancelled
                        | then cancel the workorder
                        */
                        $.post(base_url+"order/cancel_order/" + medicalRecordID + "/" + uniqueID + "/" + equipmentID, function(response){
                          var cancel_obj = $.parseJSON(response);
                          if(cancel_obj['error'] == 0)
                          {
                            $.post(base_url + "order/change_order_status/" + medicalRecordID + "/" + "cancel" + "/" + uniqueID, function(response){
                                var change_order_status_obj = $.parseJSON(response);

                                if(change_order_status_obj['error'] == 0)
                                {
                                  jAlert(change_order_status_obj['message'],"Update Response");
                                  // location.reload();
                                }
    ...
    //patient_days_date
    $('body').on('click','#patient_days' ,function(){
      var is_active = $('#patient_is_active').val();
      console.log('is_active', is_active);
      if (is_active == 1) {
        jAlert('Customer is active','Response');
      } else {
        $('#patient_days_date').show();
        $('#hide_btn_cus_date').show();
        $('#patient_days').hide();
      }
      
      console.log('patient_days_date -> Clicked!');
    });
    $('body').on('click','#hide_btn_cus_date' ,function(){
      $('#patient_days_date').hide();
      $('#hide_btn_cus_date').hide();
      $('#patient_days').show();
      console.log('patient_days_date -> Clicked!');
    });
  });
