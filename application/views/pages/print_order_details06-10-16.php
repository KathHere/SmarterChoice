...
<div class="wrapper-md">
    ...
    <div class="panel-body">
      ...
      <div class="col-xs-12 col-sm-12">
        ...
        <div class="col-xs-6 col-sm-6">
          ...
          <style type="text/css">
              .editable-click{
                border-bottom: 1px dashed #94b5e0;
              }
              @media print
              {    
                 .editable-click{
          ...
          <p class="address-edit editable-click"><?php echo $equpment_location['street']." ".$equpment_location['placenum']." ".$equpment_location['city']." ".$equpment_location['state']." ".$equpment_location['postal_code'] ?></p>
          ...
          <div class="update_address" style="display: none;">
              <form class="form-update-address">
                  <input type="hidden" value="<?php echo $addressID; ?>" name="addressID" />
                 <div class="patient-address-fields" style="margin-left:-16px;">
                    <div class="col-md-12">
                        <div class="form-group">
                          <input type="text" class="form-control grey_inner_shadow" id="" placeholder="Enter Address" name="street" style="margin-bottom:10px;" value="<?php echo $equpment_location['street']; ?>" autocomplete="off">
                        </div>
                    </div>
                     <div class="col-md-12">
                          <div class="form-group">
                             <label>Apartment # <span class="text-danger-dker">*</span></label>
                            <input type="text" class="form-control grey_inner_shadow" id="" placeholder="Apartment #, Room #" name="placenum" value="<?php echo $equpment_location['placenum']; ?>">
                          </div>
                    </div>
                    <div class="clearfix" style="margin-left: 10px;">
                      <div class="col-md-4" style="padding: 0px;">
                          <div class="form-group">
                            <input type="text" class="form-control grey_inner_shadow" id="edit_city" placeholder="City" name="city"  value="<?php echo $equpment_location['city']; ?>">
                          </div>
                      </div>
                      <div class="col-md-4" style="padding: 0px;">
                          <div class="form-group">
                             <input type="text" class="form-control grey_inner_shadow" id="edit_state" placeholder="State" name="state"  value="<?php echo $equpment_location['state']; ?>">
                          </div>
                      </div>
                      <div class="col-md-4" style="padding: 0px;">
                          <div class="form-group">
                            <input type="text" class="form-control grey_inner_shadow" id="edit_postal" placeholder="Postal" name="postal_code"  value="<?php echo $equpment_location['postal_code']; ?>">
                          </div>
                      </div>
                    </div>
                    <div class="clearfix col-md-12">
                          <div class="error-message"></div>
                          <button type="button" class="btn btn-xs btn-primary btn-save"> <i class="fa fa-pencil"></i> update</button>
                          <button type="button" class="btn btn-xs btn-danger btn-cancel"> <i class="fa fa-times"></i> cancel</button>
                    </div>
                </div>
              </form>
          </div>
          <script type="text/javascript">
              jQuery(document).ready(function($){
                  $('.address-edit').on('click',function(){
                      $(this).hide().siblings('.update_address').show();
                  });
                  $('.form-update-address').on('click','.btn-cancel',function(){
                      $(this).parents('.update_address').hide().siblings('.address-edit').show();
                  });

                 $('.form-update-address').on('click','.btn-save',function(){
                     $.post('<?php echo base_url('order/update_address')?>',$('.form-update-address').serialize(),function(data){
                        if(data.error==0)
                        {
                          $('.address-edit').text(data.data).show().siblings('.update_address').hide();
                        }
                        else
                        {
              ...
              foreach($equipments_ordered as $equipment) :
                ...
                if($equipment['equipmentID'] == 316 || $equipment['equipmentID'] == 325 || $equipment['equipmentID'] == 334 || $equipment['equipmentID'] == 343)
                {
                  if($equipment['type'] == 'Capped Item')
                  {
                    ...
                    if($equipment['original_activity_typeid'] == 2)
                    {
                      if($equipment['serial_num'] != "pickup_order_only" && $equipment['serial_num'] != "")
                      {
                        echo '<strong>Serial No. '.get_serial_num($equipment['equipmentID'], $medical_record_id).'</strong>';
                      }
                    }
                    echo '<br/>';
                  } 
  ...
  <?php 
                    ...
                    if($equipment['serial_num'] != "" && $equipment['serial_num'] != "pickup_order_only") :
                      if($act_type_id == 2 && $equipment['type'] == 'Disposable Items') :
  ?>
