...
<div class="panel-body">
  <table class="table table-striped bg-white b-a col-md-12" id="confirm_info_table" style="margin-top:0px;margin-left: 0px;">
          <thead>
            <tr>
              ...
              <?php if($this->session->userdata('account_type') == 'dme_admin' || $this->session->userdata('account_type') == 'dme_user') :?>
                <th style="width: 1px" class="action_data">Action</th>
          ...
          <tbody>
             <?php if(!empty($summaries)) :?>
                <?php foreach($summaries as $info) : ?>
                    <?php /* <?php echo form_open("",array("class"=>"update_summary_confirmed".$info['equipmentID'])) ?> */?>
                     
                    <?php if($info['categoryID'] != 3) :?>  
                      
                      <tr style="<?php if($info['canceled_order'] == 1) echo 'text-decoration:line-through' ?>">
                        <td>
                          <a href="javascript:void(0)" class="wo_num" ><?php echo substr($info['uniqueID'],4,10) ?></a>
                        </td>
                        ...
                        <td style="width:75px">
                            <input type="text" value="<?php echo $info['equipment_value'] ?>" style="width:45px;border-color:#fafafa !important;text-transform:lowercase" name="qty" class="form-control equipment_qty" />
                        ...
                        <?php if($this->session->userdata('account_type') == 'dme_admin' || $this->session->userdata('account_type') == 'dme_user') :?>
                          <td class="action_data">
                            <?php echo form_open("",array("class"=>"update_summary_confirmed".$info['equipmentID'])) ?>     
                              ...
                              <a href="javascript:void(0)" class="save_edited_summary data_tooltip" title="Save" data-value="<?php echo $info['uniqueID'] ?>" data-id="<?php echo $info['equipmentID'] ?>" style="">
      ...
      <button class="btn btn-success pull-right btn-confirm-order" data-medical-id="<?php echo $medical_id ?>" data-unique-id="<?php echo $work_order_number ?>" style="margin-right:10px">Confirm Order</button>&nbsp
