...
<?php
  if(!empty($informations)) :
    ...
    $medical_record_id = $info['medical_record_id'];
    $patientID = get_patientID($info['medical_record_id'],$info['organization_id']);

    $returned_result = '';
    $queried_data = get_patients_first_order_uniqueID_v2($info['medical_record_id'],$info['organization_id']);
    ...
    {
      $use_ptmove_address = get_ptmove_address_through_address_name_v2($patientID['patientID'],$equpment_location['street'],$equpment_location['placenum'],$equpment_location['city'],$equpment_location['state'],$equpment_location['postal_code']);
    }
    $respite_checked = "";
    if($address_type_outside['type'] == 2)
    {
      $respite_checked = "checked";
      $first_row_respite_address = get_first_row_respite_address($info['addressID'],$info['patientID']);
    }
?>
...
?>
<!-- <div class="wrapper-md visible-print-block" style="padding-top: 0px; padding-bottom: 0px">
  <div class="cancel_wrapper">
    <div class="cancel_container">CANCELED</div>
  </div>
</div> -->
<?php
  }
...
<div class="bg-light lter b-b wrapper-md work_order_header_container">
  ...
  <div class="print_content_label_div" style="margin-top:-13px;">
    <span class="print_label">Entry Time:</span> <?php echo date("F j, Y, g:i a", $info['uniqueID']) ?>
  </div>
  ...
  <div class="print_content_label_div" style="margin-top:-11px;">
    <span class="print_label">Work Order #:</span> <?php echo substr($info['uniqueID'],4,10) ?> <?php if(empty($returned_result)){ ?><span class="new-patient-icon">N</span><?php } ?>
  </div>
  ...
  <div class="panel panel-default">
    <div class="panel-body">
      ...
      <div class="print_content_label_div print_content_label_div_first">
        ...
        ?>
        <?php
          if($info['initial_order'] == 1 || ($info['pickup_order'] == 0 && $info['activity_typeid'] != 3 && $info['activity_typeid'] != 4) || $info['activity_typeid'] == 5){
        ?>
        ...
        ?>
        <?php
          if($info['original_activity_typeid'] == 2){
        ?>
      ...
      <div class="print_content_label_div print_content_label_div_second">
        ...
        <?php if($info['initial_order'] == 1 || $info['pickup_order'] == 0 && $info['activity_typeid'] != 3 && $info['activity_typeid'] != 4 && $info['activity_typeid'] != 5) :?>
          ...
          <?php echo date("m/d/Y", strtotime($info['pickup_date'])) ?>
        <?php endif;?>
        ...
        ?>
            ...
            <?php
              if(strtotime($fields['date_pickedup']) == ''):
                echo date("m/d/Y", strtotime($info['pickup_date']));
              else:
      ...
      <table class="table order_info_table" style="border:1px solid rgba(0, 0, 0, 0.55) !important; margin-top:15px;">
        <tbody style="border-top:1px solid;border-top-color:rgba(0, 0, 0, 0.55);">
          <tr>
            ...
            <td class="td_content">
              <span class="print_label">MR#: </span> <?php echo $info['medical_record_id'] ?>
            </td>
          </tr>
          <tr>
          ...
          <tr>
            ...
            <td class="td_content">
              ...
              <?php
                if($info['who_ordered_lname'] == '' && $info['who_ordered_fname'] == ''):
              ?>
                  NA
              <?php
                else:
                  echo $info['who_ordered_fname']  ." ". $info['who_ordered_lname'];
                endif;
              ?>
              </td>
            <td class="td_content" colspan="2">
              <span class="print_label">Phone: </span>
              <?php
                if($info['who_ordered_cpnum'] == ''):
              ?>
                  N/A
              <?php
                else:
                  echo $info['who_ordered_cpnum'];
                endif;
              ?>
            </td>
          </tr>
          <tr>
            <td class="td_content last_row_order_info_table" colspan="2">
              <?php
                ...
                //getting options of the equipment
              ...
              <?php
                      if(!empty($duration_)){
                        ...
                        $latest_delivery_device = get_latest_delivery_device($info['organization_id'],$info['medical_record_id']);
            ...
            <td class="td_content last_row_order_info_table" colspan="2">
              ...
              <?php
                if($info['person_confirming_order'] == ''):
                  echo "";
                else:
                  echo $info['person_confirming_order'];
                endif;
              ?>
            </td>
          </tr>
      ...
      <table class="table ordered_items_table">
        <?php
          if($info['order_status'] == "cancel") {
        ?>
        <div class="cancel_watermark">CANCELED</div>
        <?php
          }
        ?>
        <tbody style="border-top:1px solid;border-top-color:rgba(0, 0, 0, 0.55);">
          ...
          if(!empty($equipments_ordered_workorder)):
            foreach($equipments_ordered_workorder as $loop_count):
          ...
          if(!empty($equipments_ordered_workorder)):
            ...
            $item_description = "";
            foreach($equipments_ordered_workorder as $summary):
          ...
          ?>
                <tr class="rental_agreement_tr" style="height:35px;">
                  <td class="td_ordered_items_empty rental_agreement_td" colspan="7">
      ...
      <?php if($counter_printbreak > 18) { echo '<div class="printbreak"></div>'; } ?>
      ...
      <div class="col-xs-12 col-sm-12">
        <div class="row order_details_below_row_container" style="text-align:center;margin-top:45px;">
          ...
          <div class="col-xs-4 col-sm-4 work_order_created_by_content" style="display:block; margin-bottom:-18px;">
            <?php
              if($info['staff_member_fname'] == '' && $info['staff_member_fname'] == '')
              {
                echo "<span> NA </span>";
              }
              else if($info['staff_member_fname'] == 'NA' && $info['staff_member_fname'] == 'NA')
              {
                echo "<span>".$info['who_ordered_fname']  ." ". substr($info['who_ordered_lname'],0,1) .". </span>";
              }
              else
              {
                echo "<span>".$info['staff_member_fname']  ." ". substr($info['staff_member_lname'],0,1) .". </span>";
              }
            ?>
      ...
      <div class="col-xs-12 col-sm-12 last_receiver_info" style="margin-bottom:50px;">
        <div class="row" style="text-align:center;margin-top:25px;">
          ...
          <?php if($info['driver_name'] == ''): ?>
            <div class="col-xs-4 col-sm-4">
              <hr style="width:90%;border:1px solid rgba(0, 0, 0, 0.55);" />
            </div>
          <?php else:?>
            <div class="col-xs-4 col-sm-4 representative_signature_content">
              <?php echo $info['driver_name']; ?>
...
<?php endif;?>
 ...
 <?php
            ...
            //     {
            //       if($summary['equipmentID'] == 316 || $summary['equipmentID'] == 325 || $summary['equipmentID'] == 334 || $summary['equipmentID'] == 343):
            //         $item_description = $summary['key_desc'];
            //       else:
            //         $subequipment_id = get_subequipment_id($summary['equipmentID']);
            //         //gets all the id's under the order
            //         if($subequipment_id)
            ...
            //               if($summary['equipmentID'] == 55 || $summary['equipmentID'] == 20)
            //               {
            //                 $item_description = "Full Electric ".$summary['key_desc']." With ".$key['key_desc']."";
            //               }
            //               //Hi-Low Full Electric Hospital Bed equipment
            //               else if($summary['equipmentID'] == 19 || $summary['equipmentID'] == 398)
            //               {
            //                 $item_description = $summary['key_desc']." With ".$key['key_desc']."";
            //               }
            //               //Patient Lift with Sling
            //               else if($summary['equipmentID'] == 56 || $summary['equipmentID'] == 21)
            //               {
            //                 $item_description = "Patient Lift With ".$key['key_desc']."";
            //               }
            //               //Patient Lift Electric with Sling
            //               else if($summary['equipmentID'] == 353)
            //               {
            //                 $item_description = "Patient Lift Electric With ".$key['key_desc']."";
            //               }
            //               //Patient Lift Sling
            //               else if($summary['equipmentID'] == 196)
            //               {
            //                 $item_description = $key['key_desc'];
            //               }
            //               //(54 & 17) Geri Chair || (66 & 39) Shower Chair
            //               else if($summary['equipmentID'] == 54 || $summary['equipmentID'] == 17 || $summary['equipmentID'] == 66 || $summary['equipmentID'] == 39)
            //               {
            //                 $item_description = $summary['key_desc']." ".$key['key_desc']."";
            //               }
            //               // Oxygen E Portable System && Oxygen Liquid Portable
            //               else if($summary['equipmentID'] == 174 || $summary['equipmentID'] == 176 || $summary['equipmentID'] == 179)
            //               {
            ...
            //               else if($summary['equipmentID'] == 32 || $summary['equipmentID'] == 393)
            //               {
            //                 $item_description = "Oxygen ".$key['key_desc'];
            //                 break;
            //               }
            //               //(49 & 71) Wheelchair || (269 & 64) Wheelchair Reclining
            //               else if($summary['equipmentID'] == 49 || $summary['equipmentID'] == 71 || $summary['equipmentID'] == 269 || $summary['equipmentID'] == 64)
            ...
            //                       $key['key_desc'] = '24" Bariatric';
            //                     }
            //                     $item_description = $key['key_desc']." ".$summary['key_desc'];
            //                   }
            //                   else
            //                   {
            //                     $item_description = $item_description." With ".$key['key_desc'];
            //                   }
            //                 }
            //                 else
            //                 {
            //                   $item_description = '20" Wide '.$summary['key_desc'];
            //                   if($key['equipmentID'] == 86 || $key['equipmentID'] == 272)
            //                   {
            //                     $item_description = $item_description." With Elevating Legrests";
            //                     break;
            //                   }
            //                   else if($key['equipmentID'] == 87 || $key['equipmentID'] == 273)
            //                   {
            //                     $item_description = $item_description." With Footrests";
            //                     break;
            //                   }
            //                 }
            //               }
            //               else if($summary['equipmentID'] == 30)
            //               {
            //                 if(date("Y-m-d", $summary['uniqueID']) >= "2016-05-24" && strtotime($summary['pickup_date']) >= 1464073200)
            //                 {
            //                   if($count == 3)
            //                   {
            //                     $item_description = $summary['key_desc']." With ".$key['key_desc'];
            ...
            //               else if($summary['equipmentID'] == 306 || $summary['equipmentID'] == 309 || $summary['equipmentID'] == 313 || $summary['equipmentID'] == 40 || $summary['equipmentID'] == 32  || $summary['equipmentID'] == 393 || $summary['equipmentID'] == 16 || $summary['equipmentID'] == 67 || $summary['equipmentID'] == 4 || $summary['equipmentID'] == 36)
            //               {
            //                 $item_description = $summary['key_desc']."<br />";
            //                 $samp =  get_misc_item_description($summary['equipmentID'],$summary['uniqueID']);
            //                 if(strlen($samp) > 30)
            //                 {
