...
<div class="statement_invoice_inquiry_container">
    <div class="bg-light lter b-b wrapper-md">
        <h1 class="m-n font-thin h3">
            ...
            <?php } ?>
            <div class="pull-right">
                <button class="btn btn-primary btn-sm send-to-collection-btn" style="font-size:13px !important;" disabled>
                    Send to Collection
                </button>
                &nbsp;&nbsp;
            </div>
                    ...
                    <div class="col-xxs-12 col-xs-6 col-sm-6 col-md-6 select_hospice_container" style="text-align:center;">
                        ...
                        ?>
                                <select class="form-control pull-center filter_activity_status_details select2-ready" name="filter_activity_status_details" style="margin-top: 6px;border: 0px;font-size: 16px;font-weight: 600;text-align-last:center;">
                                    ...
                                    ?>
                                                <option value="<?php echo $hospice['hospiceID'] ?>" <?php if($hospiceID == $hospice['hospiceID']){ echo "selected"; } ?> >
                                    ...
                                    ?>
                                            ...
                                            ?>
                                                        <option value="<?php echo $company['hospiceID'] ?>" <?php if($hospiceID == $hospice['hospiceID']){ echo "selected"; } ?> >
                                                            <?php echo $company['hospice_account_number'] ?> - <?php echo $company['hospice_name'] ?>
                                                        </option>
                                            <?php
                                                    }
                                                endforeach;
...
<script type="text/javascript">
    ...
    var invoice_list_content = function(page, invoice_status=''){  
		...
		if(filter_from != 0 && filter_to != 0)
		{
			...
			console.log('nasud');
			$.post(base_url+"billing_statement/load_more_invoice_inquiry/" + filter_from +"/"+ filter_to +"/"+ hospiceID +"/"+ pagenum+"/"+invoice_status,"", function(response){
                ...
                for(var i = 0; i < obj.statement_invoices.length; i++) {
                    ...
                    temp_html += '<option value="us_mail" '+invoice_status_us_mail+'>US Mail</option>';
                    temp_html += '<option value="fax" '+invoice_status_fax+'>Fax</option>';
                    temp_html += '</select>';
    ...
    var get_total_payment_amount = function() {
		...
		$.post(base_url+"billing_statement/get_total_payment_amount_invoice/" + invoice_status + "/" + filter_from +"/"+ filter_to +"/0/"+ hospiceID,"", function(response){
			var obj = $.parseJSON(response);
