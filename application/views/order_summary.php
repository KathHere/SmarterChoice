<?php 
  if(!empty($informations)) :
    ...
    $queried_type = get_hospice_type($information['ordered_by']);
    if($this->session->userdata('account_type') == 'company_admin' || $this->session->userdata('account_type') == 'company_user' || $queried_type['type'] == 1)
    {
...
<div class="wrapper-md">
              ...
              <div class="media">
                <div class="media-left pull-left" style="margin-right:20px;">
                  ...
                  <?php if(!empty($information['organization_id'])){?>
                    <a href="javascript:void(0)" class="edit_patient_profile data_tooltip patient-profile-photo" title="Edit Customer Profile" data-organization-id="<?php echo $information['organization_id'] ?>" data-id="<?php echo $information['medical_record_id'] ?>">
  ...
  <?php 
          ...
          $response = get_items_for_pickup($information['medical_record_id'],$information['organization_id']);
          ...
          $patientID = get_patientID($information['medical_record_id'],$information['organization_id']);
          ...
          foreach ($patient_addresses as $key=>$value) {
            $new_response_query = get_items_for_pickup_other_address($information['medical_record_id'],$information['organization_id'],$value['id']);
  ...
  ?>
        ...
        <?php 
          ...
          */
          $equipmentID = 0;
          if(!in_array($liter_flow['parentID'],array(61,29,36)))
          {
            //get latest equipment ordered with those ID's from above and get the parent id
            $parentID = get_parent_id($information['organization_id'],$information['medical_record_id']);
            $liter_flow['parentID'] = $parentID['parentID'];
            $liter_flow['uniqueID'] = $parentID['uniqueID'];
            $equipmentID = $parentID['parentID'];
          }
          $parentID_duration = get_parent_id_duration($information['organization_id'],$information['medical_record_id']);
          if(empty($parentID_duration))
          {
            $parentID_duration = get_parent_id_duration_v2($information['organization_id'],$information['medical_record_id']);
          }
        ...
        <?php 
            ...
            if(!empty($duration_)) { 
              ...
              $latest_delivery_device = get_latest_delivery_device($information['organization_id'],$information['medical_record_id']); 
...
<script type="text/javascript">
  $(document).ready(function(){
    ...
    $.post(base_url+"order/get_customer_ordered_items/" + medical_record_id +"/"+ hospiceID,"", function(response){
