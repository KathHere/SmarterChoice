function me_message_v2(response) {
  ...
  var icons =
    "<span class='profile-photo-nav' style='height: 40px;width: 40px;margin-top: 0px;'><img style='max-width:100%' src='" +
    response.custom_icon +
    "' /></span>";
  if (response.custom_icon === undefined) {
    icons = '<span class="fa ' + status_message[status] + '"></span>';
  }
  html_data =
    '<div class="col-xs-2 preloader-body-icon">' +
    icons +
    "</div>" +
    '<div class="col-xs-8 preloader-body-content">' +
    message +
    "</div>" +
    '<div class="col-xs-2"><a class="close" data-dismiss="alert" href="javascript:void(0);"><span class="fa fa-times-circle"></span></a></div>';
  var container_open =
    '<div id="preloader-message" class="col-xs-4 response' + status + '">';
  var container_close = "</div>";
  if (jQuery("body").find("#preloader-message").length > 0) {
    jQuery("#preloader-message").html(html_data);
    jQuery("#preloader-message").removeClass("response0");
    jQuery("#preloader-message").removeClass("response1");
    jQuery("#preloader-message").removeClass("response2");
    jQuery("#preloader-message").addClass("response" + status);
  } else {
