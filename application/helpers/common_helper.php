...
function sortArray($array) {
	...
	if (count($array)) {
		array_multisort(array_map(function($element) {
			return strtolower($element['p_lname']);
...
function get_hospice_name_v2($hospiceID)
{
	$ci =& get_instance();

	$ci->load->database();

	$ci->db->select('hospice_name');
	$ci->db->from('dme_hospice');
	$ci->db->where('hospiceID', $hospiceID);
	$query = $ci->db->get();
...
function get_patient_first_order_status($patientID)
{
	$ci = get_instance();
...
function update_order_status_data($new_order_status_data,$patientID,$uniqueID)
{
	$ci =& get_instance();
	$ci->load->database();
	...
	$response = $ci->db->update('dme_order_status', $new_order_status_data);
}
...
function list_orders_scheduled_today($current_date)
{
	...
	$ci->db->select('stats.order_uniqueID as uniqueID,COALESCE((SELECT count(*) FROM dme_order_comments where dme_order_comments.order_uniqueID=stats.order_uniqueID),0) as comment_count',FALSE);
	...
	$ci->db->join('dme_patient AS patient', 'patient.patientID = stats.patientID','left');
	$ci->db->join('dme_user AS users',  'users.userID = stats.ordered_by','left');
	$ci->db->join('dme_activity_type AS act', 'act.activity_id = stats.status_activity_typeid','left');
	$ci->db->join('dme_hospice AS hosp', 'hosp.hospiceID = stats.organization_id','left');
...
//	$ci->db->join('dme_equip_category AS cat',  'cat.categoryID = equipments.categoryID','left');
	...
	$ci->db->where('stats.pickup_date',$current_date);
	$ci->db->where('stats.order_status !=', 'pending');
	$ci->db->where('stats.order_status !=', 'cancel');
	$ci->db->where('stats.order_status !=', 're-schedule');
	$ci->db->where('stats.order_status !=', 'active');
	$ci->db->where('stats.order_status !=', 'tobe_confirmed');
	$ci->db->where('stats.order_status !=', 'confirmed');
...
//get value of equipment for controller
function get_value_of_equipment_controller_v2($value, $patientID, $addressID)
{
	$ci =& get_instance();
...
function get_patient_order_v2($patientID)
{
	$ci =& get_instance();
...
function check_if_assigned_v2($equipmentID,$hospiceID)
{
	$ci =& get_instance();
...
function get_patients_first_order_uniqueID_v2($medical_record_id,$organization_id,$getfirst=false)
{
	$ci =& get_instance();
...
function get_parent_id_duration($organization_id,$medical_record_id)
{
	...
	$ci->db->where_in('o.equipmentID',array(318,319,327,328,336,337,345,346,364,365));
...
function get_ipap($organization_id,$medical_record_id)
{
	...
	$ci->db->where('activity_typeid', 1);
	$ci->db->where('order_status !=', 'cancel');
	$ci->db->where_in('equipmentID',array(109,416));
	$query = $ci->db->get();
...
function get_epap($organization_id,$medical_record_id)
{
	...
	$ci->db->where('activity_typeid', 1);
	$ci->db->where('order_status !=', 'cancel');
	$ci->db->where_in('equipmentID',array(110,417));
	$query = $ci->db->get();
...
function get_rate($organization_id,$medical_record_id)
{
	...
	$ci->db->where('activity_typeid', 1);
	$ci->db->where('order_status !=', 'cancel');
	$ci->db->where_in('equipmentID',array(111,418));
	$query = $ci->db->get();
...
function get_patient_info_details($patientID)
{
	$ci = get_instance();
...
function get_patient_first_order_v2($patientID)
{
	$ci = get_instance();
...
function get_all_patient_pickup_by_desc($patientID)
{
	$ci = get_instance();
...
function check_order_after_all_pickup_v3($orderID, $uniqueID, $patientID)
{
	$ci = get_instance();
	$ci->load->database();
...
function get_cus_move_details($patientID)
{
	$ci = get_instance();
...
function check_if_ptmove_confirmed_v2($order_uniqueID)
{
	$ci = get_instance();
	$ci->load->database();
	...
	$ci->db->select('*');
	$ci->db->from('dme_order_status as order_status');
	$ci->db->where('order_status.order_uniqueID', $order_uniqueID);
	$query = $ci->db->get()->first_row('array');
...
function get_patient_info($patientID)
{
	...
	$ci->db->select('*');
	$ci->db->from('dme_patient as patient');
	$ci->db->join('dme_order_status as orders','patient.patientID=orders.patientID');
	$ci->db->where('patient.patientID', $patientID);
	$query = $ci->db->get()->first_row('array');
...
function get_patient_info_v2($patientID,$uniqueID)
{
	...
	$ci->db->join('dme_order_status as orders','patient.patientID=orders.patientID');
	$ci->db->where('patient.patientID', $patientID);
	$ci->db->where('orders.order_uniqueID', $uniqueID);
	$query = $ci->db->get()->first_row('array');
...
function get_users_list_with_created_orders_with_date($hospiceID="",$user_location,$current_date = "")
{
	$ci = get_instance();
	$ci->load->database();

	$ci->db->select('user.userID,user.firstname,user.lastname,user.group_id,order_status.*,patient.p_fname,patient.p_lname,patient.medical_record_id,act.*');
	$ci->db->from('dme_user as user');
	$ci->db->join("dme_order_status as order_status","order_status.ordered_by=user.userID");
	$ci->db->join("dme_patient as patient","patient.patientID=order_status.patientID");
	$ci->db->join("dme_activity_type as act","act.activity_id=order_status.status_activity_typeid");

	$ci->db->where("patient.ordered_by != ", 13);

	if(!empty($hospiceID) && $hospiceID != 0)
	{
		$ci->db->where('order_status.organization_id', $hospiceID);
	}

	if ($user_location != 0) {
		$ci->db->where('user.user_location', $user_location);
	} else {
		$ci->db->where('user.user_location !=', 0);
	}
	

	if(!empty($current_date))
    {
    	$ci->db->where("DATE_FORMAT(order_status.date_ordered,'%Y-%m-%d')",$current_date);
    }

	$ci->db->order_by('user.lastname', "ASC");
...
function get_ptmove_address($medical_id, $uniqueID)
{
	...
	$ci->db->from('dme_patient as pat');
	$ci->db->join('dme_sub_ptmove as pt','pat.medical_record_id = pt.medical_record_id');
	$ci->db->join('dme_order_status as orders','pt.medical_record_id = orders.medical_record_id');
...
function get_address_type_v3($delivery_address_id)
{
	$ci = get_instance();
	$ci->load->database();

	$ci->db->select('type,id');
	$ci->db->from('dme_patient_address');
	$ci->db->where('id', $delivery_address_id);
...
function get_address_type_v2($patientID="")
{
	$ci = get_instance();
	$ci->load->database();
...
//add september 5, 2018 by Adrian
function get_address_id($patientID = '')
{
    $ci = get_instance();
    $ci->load->database();
...
function get_cusmove_address_id($patientID = '')
{
    $ci = get_instance();
    $ci->load->database();
...
function get_delivery_to_type($patientID = '')
{
    $ci = get_instance();
    $ci->load->database();

    $ci->db->select('deliver_to_type');
    $ci->db->from('dme_order_status');
    $ci->db->where('patientID', $patientID);
    $ci->db->where('cancelled_order', 0);
    $ci->db->where('canceled_from_confirming', 0);
    $ci->db->order_by('date_ordered', 'DESC');
...
function get_equipmentID_of_parentID($equipmentID)
{
    $ci = get_instance();
    $ci->load->database();
...
function get_ptmove_addresses_ID_v2($patientID)
{
	...
	$ci->db->from('dme_patient_address as address');
	$ci->db->join('dme_order_status as orders','orders.addressID = address.id');
	$ci->db->where('address.patient_id', $patientID);
	$ci->db->where('orders.order_status !=', 'cancel');
	$ci->db->where('address.type', 1);
...
function get_patientID_v2($id,$hospice_id)
{
	$ci = get_instance();
	...
	$ci->db->from('dme_patient');
	$ci->db->where('medical_record_id', $id);
	$ci->db->where('ordered_by', $hospice_id);
	$query = $ci->db->get();

	return $query->first_row('array');
}

function get_latest_dme_order_row($patientID)
{
	$ci = get_instance();
...
/** Check if logistics user logged in **/
function is_lgsts_user_logged_in()
{
	$CI =& get_instance();

	$lgsts_user = $CI->session->userdata('lgsts_user');

	if(empty($lgsts_user)) {
		redirect(base_url('main/lgsts_login'));
	} else if (empty($lgsts_user['id'])) {
		redirect(base_url('main/lgsts_login'));
	}
}
...
function get_assigned_equipment_v2($hospiceID="")
{
	$ci =& get_instance();
	$ids = array();
	if(!empty($hospiceID))
...
function get_hospices_v2($user_location)
{
	...
	if ($user_location != 0) { // Added 08/17/2021
		$CI->db->where('dme_hospice.account_location', $user_location);
	} else {
		$CI->db->where('dme_hospice.account_location !=', 0);
	}
	...
	foreach($query->result_array() as $key=>$c)
	{
		$data_array[$c['hospiceID']] = array(
			...
			'hospice_name'			=> $c['hospice_name'],
			'contact_num'				=> $c['contact_num'],
			'hospice_account_number' => $c['hospice_account_number']
		);
	}
...
function get_hospices_v3($user_location)
{
	$CI =& get_instance();
...
function get_new_patient_list($current_date,$from,$to,$hospiceID)
{
    ...
    $CI->db->join("dme_order_status as orders","pat_address.id=orders.addressID");
    ...
    if(!empty($from) && !empty($to))
    {
	  ...
	  //           $CI->db->where("orders.order_status", "active");
			...
			if($to == $current_day || $to > $current_day)
			{
				...
				$CI->db->where("orders.order_uniqueID > ",$final_start);
				$CI->db->where("orders.order_uniqueID < ",$final_end);
			}
...
function get_new_patient_list_v4($current_date,$from,$to,$hospiceID,$account_location,$pagination_details=array(),$getcount=false)
{
	$CI =& get_instance();
...
function get_user_orders_made_today($userID,$current_date)
{
    ...
    $CI->db->from("dme_patient as pat");
    $CI->db->join("dme_order_status as orders","pat.patientID=orders.patientID");
...
function get_ptmove_list_v2($current_date,$from,$to,$hospiceID,$account_location,$pagination_details=array(),$getcount=false)
{
	$CI =& get_instance();
...
//   if($new_format_to == $new_format_current_date." 23:59:59")
//   {
//   	$CI->db->where("orders.order_status", "active");
//   }
//   if(!empty($from) && !empty($to))
...
function get_respite_list_v2($current_date,$from,$to,$hospiceID,$account_location,$pagination_details=array(),$getcount=false)
{
	$CI =& get_instance();
...
function get_patient_new_item_list_v2($current_date,$from,$to,$hospiceID,$account_location,$pagination_details=array(),$getcount=false)
{
	$CI =& get_instance();
	$CI->load->database();
    ...
    if($getcount){
      $CI->db->select("count(DISTINCT orders.uniqueID,equip.equipmentID) as total",false);
    }
    else{
      $CI->db->select("orders.*,equip.*,pat.p_fname,pat.p_lname,count(DISTINCT equip.equipmentID) as total_items",false);
      if(isset($pagination_details['offset']) && isset($pagination_details['limit'])){
          $CI->db->limit($pagination_details['limit'],$pagination_details['offset']);
      }
    }
...
// $current_day = date('Y-m-d');
// $CI->db->where("(orders.actual_order_date >='{$from}' AND orders.actual_order_date <='{$to}' )",null,false);
// $subquery_where .= " AND (orders.actual_order_date>='{$from}' AND orders.actual_order_date <='{$to}' ) ";
// $subquery_exchange = "COALESCE(
//                       (SELECT count() as total FROM dme_order as ordl
//                           JOIN dme_equipment as equip ON ordl.equipmentID = equip.equipmentID
//                           WHERE
//                             equip.parentID=0
//                             AND ordl.patientID=orders.patientID
//                             $subquery_where
//                       ),0) as total_items ";
// echo $CI->db->last_query();
...
function get_patient_pickup_list_v2($current_date,$from,$to,$hospiceID,$account_location,$pagination_details=array(),$getcount=false,$reason="")
{
	$CI =& get_instance();
...
// 	$CI->db->where("orders.order_status", "active");
// }
// $CI->db->where("orders.status_activity_typeid",2);
// if(!empty($from) && !empty($to))
// {
// 	$current_day = date('Y-m-d');
	// $CI->db->where("DATE_FORMAT(orders.actual_order_date,'%Y-%m-%d') >= ",$from);
...
function get_patient_exchange_list_v2($current_date,$from,$to,$hospiceID,$account_location,$pagination_details=array(),$getcount=false)
{
	//echo $current_date.$from.$to.$account_location;
	  $CI =& get_instance();
	  $CI->load->database();
...
function list_active_patients_by_hospice_and_residence_v2($hospiceID,$residence_status_name)
{
	...
	$ci->load->database();
	$ci->db->select('patient.*, orders.organization_id, orders.medical_record_id, orders.deliver_to_type, orders.order_uniqueID');
	$ci->db->from('dme_order_status AS orders');
	$ci->db->join('dme_patient AS patient', 'patient.patientID = orders.patientID','left');
	$ci->db->join('dme_activity_type AS act', 'act.activity_id = orders.status_activity_typeid','left');
...
function check_if_all_pickups_v3($patientID)
{
	$ci = get_instance();
	$ci->load->database();
...
function get_count_status_new()
{
	...
	{
		// $ci->db->where('orders.organization_id', $ci->session->userdata('group_id'));
		$where = " AND stats.organization_id = ".$ci->session->userdata('group_id');
	}
...
function get_count_status_new_v2($account_location)
{
	...
	if($ci->session->userdata('account_type') != 'dme_admin' && $ci->session->userdata('account_type') != "dme_user" && $ci->session->userdata('account_type') != 'dispatch' && $ci->session->userdata('account_type') != 'sales_rep' && $ci->session->userdata('account_type') != 'biller' && $ci->session->userdata('account_type') != 'customer_service' && $ci->session->userdata('account_type') != 'rt' && $ci->session->userdata('account_type') != 'distribution_supervisor')
	{
		// $where = " AND stats.organization_id = ".$ci->session->userdata('group_id'). " AND hospice.account_location = ".$account_location;
...
function get_count_status($status="pending",$additional = array())
{
	...
	if($ci->session->userdata('account_type') != 'dme_admin' && $ci->session->userdata('account_type') != "dme_user" && $ci->session->userdata('account_type') != 'dispatch' && $ci->session->userdata('account_type') != 'sales_rep' && $ci->session->userdata('account_type') != 'biller' && $ci->session->userdata('account_type') != 'customer_service' && $ci->session->userdata('account_type') != 'rt' && $ci->session->userdata('account_type') != 'distribution_supervisor')
	{
		$ci->db->where('stats.organization_id', $ci->session->userdata('group_id'));
	}
	$ci->db->group_by('stats.status_activity_typeid');
	$ci->db->group_by('stats.order_uniqueID');
	if(!empty($status))
	{
		$ci->db->where('stats.order_status',$status);
	}
//print_r($additional);exit;
	if(!empty($additional))
	{
		...
		{
      $value = str_replace("orders.","stats.",$value);
      $key = str_replace("orders.","stats.",$key);
			if(!$value)
			{
...
function get_count_status_v2($status="pending",$additional = array(),$account_location)
{
	...
	$ci->db->from('dme_order_status as stats');
//	$ci->db->join('dme_order as orders', 'stats.order_uniqueID=orders.uniqueID','left');
	$ci->db->join('dme_hospice as hospice', 'stats.organization_id=hospice.hospiceID','left');
...
function list_group_and_group_commercial($user_location=array())
{
	$CI =& get_instance();
