jQuery(document).ready(function(){
    ...
    $('select.filter_reports_by_hospice').on('change', function (e) {
		...
		monthly_activityreport(hospiceID,search_from,search_to);
		monthly_patienceresidence_report(hospiceID,search_from,search_to);
		get_high_cost_customers(hospiceID);
	...
	$('body').on('click','.activity_status_link',function(){
		...
		modalbox(base_url + 'report/view_each_activity_status/' + activity_status_name+'/'+viewed_hospiceID+"/"+date_from+"/"+date_to,{
	...
	var get_high_cost_customers = function(hospiceID)
	{
		console.log('hospiceID hey', hospiceID);
		if(typeof hospiceID !="undefined" && hospiceID != 0)
		{
			$('body').find('.high_cost_customers_container').css('display','block');
			$('body').find('.high_cost_customers_loader').css('display','block');
			$('body').find('.first_five_high_cost_customers').css('display','none');
			$('body').find('.second_five_high_cost_customers').css('display','none');
			$.get(base_url+"report/high_cost_customers/"+hospiceID,function(response){
				console.log('high cost customers', response);
				$('body').find('.high_cost_customers_loader').css('display','none');
				$('body').find('.first_five_high_cost_customers').css('display','block');
				$('body').find('.second_five_high_cost_customers').css('display','block');
				$('body').find(".first_five_high_cost_customers").html("");
				$('body').find(".second_five_high_cost_customers").html("");
				for(var i=0; i<response.data.high_cost_customers.length; i++)
				{
					if (i < 5) {
						if (i == 0) {
							$('body').find(".first_five_high_cost_customers").append('<div class="col-xs-3 col-sm-2 col-md-1"></div>');
						}
						$('body').find(".first_five_high_cost_customers").append('<div class="col-xs-6 col-sm-4 col-md-2"> <div class="row mt10 mb10" style="font-size:15px; padding-left: 20px;"> <a target="_blank" href="'+ base_url +'order/patient_profile/' + response.data.high_cost_customers[i].medical_record_id +'/'+ response.data.high_cost_customers[i].hospiceID +'"> Mr#'+response.data.high_cost_customers[i].medical_record_id+' <br /> '+response.data.high_cost_customers[i].p_lname+', '+response.data.high_cost_customers[i].p_fname+' : $'+response.data.high_cost_customers[i].total.toFixed(2)+' </a></div></div>');
					} else {
						if (i == 5) {
							$('body').find(".second_five_high_cost_customers").append('<div class="col-xs-3 col-sm-2 col-md-1"></div>');
						}
						$('body').find(".second_five_high_cost_customers").append('<div class="col-xs-6 col-sm-4 col-md-2"> <div class="row mt10 mb10" style="font-size:15px; padding-left: 20px;"> <a target="_blank" href="'+ base_url +'order/patient_profile/' + response.data.high_cost_customers[i].medical_record_id +'/'+ response.data.high_cost_customers[i].hospiceID +'"> Mr#'+response.data.high_cost_customers[i].medical_record_id+' <br /> '+response.data.high_cost_customers[i].p_lname+', '+response.data.high_cost_customers[i].p_fname+' : $'+response.data.high_cost_customers[i].total.toFixed(2)+' </a></div></div>');
					}
				}
			});
		} else {
			$('body').find('.high_cost_customers_container').css('display','none');
		}
	}

	var hospiceIDReportPage = $("body").find("#hospiceID_report_page").val();
	console.log('hospiceIDReportPage', hospiceIDReportPage);
	if (typeof hospiceIDReportPage != undefined && hospiceIDReportPage != 0 && hospiceIDReportPage != "") {
		get_high_cost_customers(hospiceIDReportPage);
	}
	...
	$('body').on('click','.residence_status_link',function(){
		...
		var search_to = $("body").find("#search_to").val();
	  	$("body").find(".modal-dialog").addClass("modal-lg");
		modalbox(base_url + 'report/view_each_residence_status/' + residence_status_name +'/'+ viewed_hospiceID+'/'+search_from+'/'+search_to,{
		...
		if(data_name == "new_pt")
	  	{
			...
			$("body").find(".hover_tbody").html('<tr><td colspan="2" style="text-align:center;"> <i class="fa fa-spin fa-spinner"></i></td></tr>');
	  		...
	  		$.post(base_url+"report/sort_activity_status_details/"+ date_from +"/"+ date_to +"/"+ selected_hospice+"/"+ data_name +"/1/5","", function(response){
	...
	$('body').on('mouseover','.residence_status_link',function(){
		...
		date_to = (date_to=="")? 0 : date_to;
		date_from = (date_from=="")? 0 : date_from;
		if(data_name == "assisted_living")
	  	{
			...
			$("body").find(".hover_sample_div").find(".hover_tbody").html('<tr><td colspan="2" style="text-align:center;"> <i class="fa fa-spin fa-spinner"></i></td></tr>');
				$.post(base_url+"report/sort_residence_status_details/" + date_from +"/"+ date_to +"/"+ selected_hospice +"/"+ data_name+"/1/5","", function(response){
	  			var obj = $.parseJSON(response);
	  			var count = 0;
	  			...
	  			$("body").find(".hover_tbody").html(table_content);
	  			$("body").find(".total_patient_sample_count").html(obj.pagination_details.total_records);
			});
	  	}
